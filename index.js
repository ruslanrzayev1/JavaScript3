...
// let resultsNew = results.filter(item => item.order.customerId == "ERNSH");

// console.log(resultsNew.length);


// "3) employeeİd- görə sortlayın"

// let resultsNew = results.filter(item => item.order.employeeId);

// let resultsNew2 = resultsNew.sort((a, b) => a.order.employeeId - b.order.employeeId);

// console.log(resultsNew2);


// "4) OrderDetailsi 3-dən çox olanları console-a yazdır"

// let resultsNew = results.filter(item => item.orderDetails.length > 3);
...
// let resultsNew = results.filter(item => item.order.id == 11075)
